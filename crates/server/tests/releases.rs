...
async fn release_api_is_paginated_conflict_safe_and_tenant_scoped(pool: sqlx::PgPool) {
    ...
    assert_eq!(server::database::REQUIRED_MIGRATION, 7);
...
async fn ingestion_builds_idempotent_release_summaries_and_complete_diff(pool: sqlx::PgPool) {
    ...
    assert_eq!(persist_batch(&pool, context, &events).await.unwrap(), 5);
    ...
    assert_eq!(counts.iter().map(|(_, count)| *count).sum::<i64>(), 6);
    assert!(
        counts
            .iter()
            .any(|(id, count)| *id == baseline_id && *count == 2)
    );
    assert!(
        counts
            .iter()
            .any(|(id, count)| *id == target_id && *count == 4)
    );
