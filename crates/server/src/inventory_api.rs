...
fn validate_kind(kind: Option<&str>) -> Result<(), InventoryApiError> {
    if kind.is_none_or(|value| {
        matches!(
            value,
            "process" | "destination" | "domain" | "syscall" | "inbound_endpoint"
        )
    }) {
        Ok(())
    } else {
        Err(InventoryApiError::Invalid(
            "kind must be process, destination, domain, syscall, or inbound_endpoint".into(),
        ))
    }
}
